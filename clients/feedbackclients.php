...
// CHANGE 'u.name' BELOW TO MATCH YOUR EXACT USERS TABLE COLUMN NAME (E.G., full_name)
$feedback_query = "SELECT f.id, f.user_id, f.rating, f.feedback_text, f.created_at, name AS username 
                   FROM feedback f 
                    ...
                    <?php while ($row = $feedback_result->fetch_assoc()): ?>
                        <div class="glass-card community-card">
                            <div class="community-header-row">
                                <strong><?php echo strtoupper(htmlspecialchars($row['username'])); ?></strong>
