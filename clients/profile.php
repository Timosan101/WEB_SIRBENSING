...
// 4. FETCH ACTIVE ORDERS (STATUS: PENDING / PROCESSING / SHIPPED)
$active_orders = [];
...
// 5. FETCH HISTORY ORDERS (STATUS: DELIVERED)
$history_orders = [];
$history_query = "SELECT o.id, o.quantity, o.order_date, o.delivery_date, 
                         p.name AS product_name, p.category, p.price 
                  FROM orders o 
                  JOIN products p ON o.product_id = p.id 
                  WHERE o.user_id = ? AND o.status = 'DELIVERED' 
            ...
            <?php if ($history_orders && $history_orders->num_rows > 0): ?>
                <?php while ($past = $history_orders->fetch_assoc()): ?>
                    ...
                    <tr>
                        ...
                        <td><?php echo $past['delivery_date']; ?></td>
                    </tr>
                <?php endwhile; ?>
            <?php else: ?>
                <tr>
